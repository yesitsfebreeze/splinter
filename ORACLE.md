...
- 2026-07-17 — The release is created once in the prepare job; build jobs only upload (plain `gh`, replacing the softprops action). Five concurrent matrix jobs racing to create the same release split v0.1.19's assets across duplicate releases — v0.1.18 had merely won the timing lottery. Decided by: "Fix bugs on sight" and "Builtin before built" (`gh` ships on every runner). Supersedes: per-job release creation via softprops/action-gh-release.
